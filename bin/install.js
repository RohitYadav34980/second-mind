...
-  already used by option '${matchingOption.flags}'`);
      ...
      _prepareUserArgs(argv, parseOptions) {
        ...
        if (argv === void 0 && parseOptions.from === void 0) {
          if ((_a = process10.versions) == null ? void 0 : _a.electron) {
...
async function copySkillFolder(sourceSkillDir, targetSkillDir) {
  await (0, import_promises.mkdir)(targetSkillDir, { recursive: true });
  try {
    await (0, import_promises.cp)(sourceSkillDir, targetSkillDir, { recursive: true });
  } catch {
    if (process.platform === "win32") {
...
async function copyFolders(sourceDir, targetDir, aiType) {
  const copiedFolders = [];
  const sourceSkillPath = (0, import_node_path.join)(sourceDir, "second-mind");
  if (!await exists(sourceSkillPath)) {
    throw new Error(`Could not find skill folder 'second-mind' inside ${sourceDir}`);
  }
  const platforms = aiType === "all" ? Object.keys(PLATFORM_CONFIGS) : [aiType];
  for (const plat of platforms) {
    const config = PLATFORM_CONFIGS[plat];
    const targetSkillPath = (0, import_node_path.join)(targetDir, config.root, config.skillPath);
    try {
      if (await exists(targetSkillPath)) {
        await (0, import_promises.rm)(targetSkillPath, { recursive: true, force: true });
      }
...
function detectAIType(cwd = process.cwd()) {
  ...
  for (const [key, config] of Object.entries(PLATFORM_CONFIGS)) {
    if ((0, import_node_fs.existsSync)((0, import_node_path2.join)(cwd, config.root))) {
      detected.push(key);
    }
  }
  let suggested = null;
...
try {
  ...
  currentDir = (0, import_node_path3.dirname)(filename);
} catch {
  currentDir = __dirname;
}
function getBundledDir() {
  let dir = (0, import_node_path3.join)(currentDir, "..");
  if ((0, import_node_fs2.existsSync)((0, import_node_path3.join)(dir, "second-mind", "SKILL.md"))) {
    return dir;
  }
  dir = (0, import_node_path3.join)(currentDir, "..", "..", "..");
  if ((0, import_node_fs2.existsSync)((0, import_node_path3.join)(dir, "second-mind", "SKILL.md"))) {
    return dir;
  }
  return process.cwd();
...
async function installCommand(options) {
  ...
  if (!aiType) {
    ...
    if (detected.length > 0) {
      logger.info(`Detected active AI assistant environments: ${detected.map((t) => source_default.cyan(t)).join(", ")}`);
    }
  ...
  const targetDir = isGlobal ? (0, import_node_os2.homedir)() : process.cwd();
  try {
    ...
    console.log(source_default.dim('  2. Try typing command: "/second-mind init" or "second-mind init"'));
    console.log();
  } catch (error) {
...
async function removeSkillDir(baseDir, aiType) {
  ...
  if (!config)
    return [];
  const skillDir = (0, import_node_path4.join)(baseDir, config.root, config.skillPath);
  const removed = [];
  try {
    await (0, import_promises2.stat)(skillDir);
    await (0, import_promises2.rm)(skillDir, { recursive: true, force: true });
...
async function uninstallCommand(options) {
  ...
  const isGlobal = !!options.global;
  const baseDir = isGlobal ? (0, import_node_os3.homedir)() : process.cwd();
  ...
  const { confirmed } = await (0, import_prompts2.default)({
    type: "confirm",
    name: "confirmed",
    message: `Remove Second Mind skill for ${source_default.cyan(getAITypeDescription(aiType))} from ${locationLabel}?`,
    initial: false
  });
  if (!confirmed) {
    logger.warn("Uninstall cancelled");
...
try {
  ...
  currentDir2 = (0, import_node_path5.dirname)(filename);
} catch {
  currentDir2 = __dirname;
}
var pkgPath = (0, import_node_path5.join)(currentDir2, "../package.json");
...
program2.name("second-mind").description("CLI to install Second Mind skill for AI coding assistants").version(pkg.version);
program2.command("install").description("Install Second Mind skill to current project").option("-a, --ai <type>", `AI assistant type (${AI_TYPES.join(", ")})`).option("-f, --force", "Overwrite existing files").option("-g, --global", "Install globally to home directory (~/) instead of current project").action(async (options) => {
...
program2.command("uninstall").description("Remove Second Mind skill from current project or globally").option("-a, --ai <type>", `AI assistant type (${AI_TYPES.join(", ")})`).option("-g, --global", "Uninstall from home directory (~/) instead of current project").action(async (options) => {
  if (options.ai && !AI_TYPES.includes(options.ai)) {
    console.error(`Invalid AI type: ${options.ai}`);
    console.error(`Valid types: ${AI_TYPES.join(", ")}`);
    process.exit(1);
...
var availableCommands = ["install", "uninstall", "-h", "--help", "-v", "--version"];
